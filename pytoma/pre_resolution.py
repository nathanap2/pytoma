...
def pre_resolve_path_rules(cfg: Config) -> tuple[Config, list[str]]:
    ...
    for idx, r in enumerate(rules):
        ...
        prio_action = (
            3 if kind == "hide" else
            2 if kind.startswith("file:") else
            1
        )
        meta.append({
            "idx": idx, "rule": r,
            "is_path": is_path, "is_lit": is_lit,
    ...
    for L in lit_items:
        ...
        for G in glob_items:
            ...
            if fnmatch.fnmatchcase(Ls, Gs):
                ...
                if L["prio_action"] >= 2 and G["prio_action"] >= 2 and L["rule"].mode != G["rule"].mode:
                    warnings.append(
    ...
    while changed:
        ...
        for g, l in list(must_after):
            if pos[g] < pos[l]:
                ...
                order.remove(g); order.insert(pos[l]+1, g)
                pos = {idx: i for i, idx in enumerate(order)}
                changed = True
