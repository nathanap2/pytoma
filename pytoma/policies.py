...
def hide() -> Action: return Action("hide")
def full() -> Action: return Action("full")
def sig_doc() -> Action: return Action("sig+doc")
def sig() -> Action: return Action("sig")
def levels(k: int) -> Action: return Action("levels", {"k": int(k)})

# (Optional) action registry if you want centralized validation
_VALIDATORS: Dict[str, Callable[[Action], None]] = {}

def register_action(kind: str, validator: Optional[Callable[[Action], None]] = None) -> None:
